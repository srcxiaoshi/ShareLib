...
//    [dic safe_setObject:[NSString stringWithFormat:@"%ld",recordTime] forKey:@"ts"];//这里是时间戳
    ...
    [SRCNetworkWithAF requestGetMethodWithPath:@"https://is.snssdk.com/network/get_network/" parameters:nil withProgress:nil success:^(BOOL isSuccess, NSString *response) {
        //是不是要重传，由外面用户决定
        if(block)
        {
